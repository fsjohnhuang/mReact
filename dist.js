...
const createElement = (type, props, ...children) => {
  return {
    type,
    props,
    children: children.filter(child => child != undefined).flatMap(x => x).map(child => typeof child === 'object' ? child : { type: 'TEXT_ELEMENT', props: { nodeValue: child } })
  };
};
...
const App = () => createElement(
  ...
  Array.from(Array(message).keys()).map(i => createElement(
    'div',
    { key: i },
    i
  ))
